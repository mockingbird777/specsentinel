...
// src/errors.ts
var SpecSentinelError = class extends Error {
...
// src/types.ts
var severityOrder = ["info", "low", "medium", "high", "critical"];
...
// src/config/config.ts
var formats = ["terminal", "json", "markdown", "sarif", "html"];
async function loadConfig(filePath) {
  let value;
  try {
    value = import_yaml.default.parse(await (0, import_promises.readFile)(filePath, "utf8"));
  } catch (error) {
  ...
  if (value.failOn !== void 0 && !severityOrder.includes(value.failOn)) {
    throw new SpecSentinelError(`Invalid failOn value in ${filePath}`);
  }
  if (value.format !== void 0 && !formats.includes(value.format)) {
    throw new SpecSentinelError(`Invalid format value in ${filePath}`);
  }
  if (value.ignoreRules !== void 0 && (!Array.isArray(value.ignoreRules) || !value.ignoreRules.every((item) => typeof item === "string"))) {
    throw new SpecSentinelError(`ignoreRules in ${filePath} must be an array of rule IDs`);
  }
  if (value.ignores !== void 0 && (!Array.isArray(value.ignores) || !value.ignores.every(validIgnore))) {
    throw new SpecSentinelError(`ignores in ${filePath} must contain { rule, location? } entries`);
  }
  return value;
}
function validIgnore(value) {
  return isObject(value) && typeof value.rule === "string" && (value.location === void 0 || typeof value.location === "string");
}
function wildcardMatches(pattern, value) {
  const escaped = pattern.replace(/[.+?^${}()|[\]\\]/g, "\\$&").replaceAll("*", ".*");
  return new RegExp(`^${escaped}$`).test(value);
}
function filterIgnored(changes, config, cliRules = []) {
  const ruleIds = /* @__PURE__ */ new Set([...config.ignoreRules ?? [], ...cliRules]);
  return changes.filter((change) => {
    if (ruleIds.has(change.ruleId)) return false;
    return !(config.ignores ?? []).some(
      (ignore) => wildcardMatches(ignore.rule, change.ruleId) && (ignore.location === void 0 || wildcardMatches(ignore.location, change.location))
    );
  });
}

// src/ref.ts
function decodePointerPart(part, reference) {
  if (/~(?:[^01]|$)/.test(part)) {
...
var rules = {
  ...
  "PARAM_ENUM_NARROWED": {
    id: "PARAM_ENUM_NARROWED",
    title: "Parameter enum narrowed",
    defaultSeverity: "high",
...
function schemaType(schema) {
  ...
  if (schema.type !== void 0) return schema.type;
...
function schemaPairKey(before, after) {
  if (!isObject(before) || !isObject(after)) return void 0;
...
function compareRequestSchema(beforeValue, afterValue, context) {
  ...
  for (const property of afterRequired) {
    if (!beforeRequired.has(property)) {
      context.changes.push({
        ruleId: "REQUEST_PROPERTY_REQUIRED",
        severity: "high",
        location: `${context.location}/properties/${pointerPart(property)}`,
        message: `Request property '${property}' is now required.`,
        before: false,
        after: true
      });
    }
  ...
  if (before.items !== void 0 && after.items !== void 0) {
...
function compareResponseSchema(beforeValue, afterValue, context) {
  if ((context.depth ?? 0) > 40) return;
  const key = schemaPairKey(beforeValue, afterValue);
  if (key && context.seen.has(key)) return;
  if (key) context.seen.add(key);
  ...
  for (const [property, childBefore] of Object.entries(beforeProperties)) {
    ...
    if (!(property in afterProperties)) {
      context.changes.push({
        ruleId: "RESPONSE_PROPERTY_REMOVED",
        severity: "high",
        location: childLocation,
        message: `Response property '${property}' was removed.`,
        before: childBefore,
        after: void 0
      });
    } else {
...
// src/diff/security.ts
function normalize(value) {
  if (value === void 0 || Array.isArray(value) && value.length === 0) return [/* @__PURE__ */ new Map()];
...
function noStricter(candidate, baseline) {
...
function add(changes, ruleId, location, message, before, after) {
  const definition = rules[ruleId];
  if (!definition) throw new Error(`Unknown rule ${ruleId}`);
  const change = { ruleId, severity: definition.defaultSeverity, location, message };
  if (before !== void 0) change.before = before;
  if (after !== void 0) change.after = after;
  changes.push(change);
}
...
function compareParameters(baseline, candidate, beforePath, afterPath, beforeOperation, afterOperation, operationLocation, changes) {
  ...
  for (const [key, afterEntry] of afterParameters) {
    ...
    if (!beforeParameter) {
      if (afterParameter.required === true) {
        add(changes, "PARAM_REQUIRED_ADDED", location, `Required ${where} parameter '${name}' was added.`, void 0, afterParameter);
      }
      continue;
    }
    ...
    const afterEnum = Array.isArray(afterSchema?.enum) ? afterSchema.enum : void 0;
    const removedValues = beforeEnum && afterEnum ? beforeEnum.filter((value) => !afterEnum.some((candidateValue) => JSON.stringify(candidateValue) === JSON.stringify(value))) : beforeEnum === void 0 && afterEnum ? ["<previously unrestricted>"] : [];
...
function compareRequestBody(baseline, candidate, beforeOperation, afterOperation, operationLocation, changes) {
  ...
  for (const [mediaType, beforeMediaValue] of Object.entries(beforeContent)) {
    ...
    if (!(mediaType in afterContent)) {
      add(changes, "REQUEST_CONTENT_REMOVED", contentLocation, `Request media type '${mediaType}' was removed.`, beforeMediaValue, void 0);
      continue;
    }
    const beforeMedia = resolveObject(baseline, beforeMediaValue);
    const afterMedia = resolveObject(candidate, afterContent[mediaType]);
    if (!beforeMedia || !afterMedia || beforeMedia.schema === void 0 || afterMedia.schema === void 0) continue;
...
function compareResponses(baseline, candidate, beforeOperation, afterOperation, operationLocation, changes) {
  ...
  for (const [status, beforeResponseValue] of Object.entries(beforeResponses)) {
    ...
    if (!(status in afterResponses)) {
      add(changes, "RESPONSE_REMOVED", responseLocation, `Response '${status}' was removed.`, beforeResponseValue, void 0);
      continue;
    }
    ...
    for (const [mediaType, beforeMediaValue] of Object.entries(beforeContent)) {
      ...
      if (!(mediaType in afterContent)) {
        add(changes, "RESPONSE_CONTENT_REMOVED", contentLocation, `Response media type '${mediaType}' was removed.`, beforeMediaValue, void 0);
        continue;
      }
      const beforeMedia = resolveObject(baseline, beforeMediaValue);
      const afterMedia = resolveObject(candidate, afterContent[mediaType]);
      if (!beforeMedia || !afterMedia) continue;
      if (beforeMedia.schema !== void 0 && afterMedia.schema === void 0) {
        add(changes, "RESPONSE_CONTENT_REMOVED", `${contentLocation}/schema`, `Response schema for '${mediaType}' was removed.`, beforeMedia.schema, void 0);
      } else if (beforeMedia.schema !== void 0 && afterMedia.schema !== void 0) {
...
function diffOpenApi(input2) {
  const changes = [];
  const beforePaths = records(input2.baseline.paths);
  const afterPaths = records(input2.candidate.paths);
  for (const [path, beforePathValue] of Object.entries(beforePaths)) {
    ...
    if (!(path in afterPaths)) {
      add(changes, "PATH_REMOVED", pathLocation, `Path '${path}' was removed.`, beforePathValue, void 0);
      continue;
    }
    const beforePath = resolveObject(input2.baseline, beforePathValue);
    const afterPath = resolveObject(input2.candidate, afterPaths[path]);
    if (!beforePath || !afterPath) continue;
    for (const method of methods) {
      const beforeOperation = resolveObject(input2.baseline, beforePath[method]);
      if (!beforeOperation) continue;
      const operationLocation = `${pathLocation}/${method}`;
      const afterOperation = resolveObject(input2.candidate, afterPath[method]);
      if (!afterOperation) {
        add(changes, "OPERATION_REMOVED", operationLocation, `${method.toUpperCase()} ${path} was removed.`, beforeOperation, void 0);
        continue;
      }
      compareParameters(input2.baseline, input2.candidate, beforePath, afterPath, beforeOperation, afterOperation, operationLocation, changes);
      compareRequestBody(input2.baseline, input2.candidate, beforeOperation, afterOperation, operationLocation, changes);
  ...
  return makeResult(input2, changes);
}
...
function parseOpenApi(text, source = "<memory>") {
  let parsed;
  try {
    parsed = import_yaml2.default.parse(text);
  } catch (error) {
...
async function compareFiles(options) {
  const [baseline, candidate, config] = await Promise.all([
    loadOpenApi(options.baseline),
    loadOpenApi(options.candidate),
    options.config ? loadConfig(options.config) : Promise.resolve({})
  ]);
...
<meta property="og:type" content="website"><meta property="og:title" content="SpecSentinel \xB7 OpenAPI contract report">
...
<meta name="twitter:card" content="summary"><meta name="twitter:title" content="SpecSentinel \xB7 OpenAPI contract report">
...
<title>SpecSentinel \xB7 OpenAPI contract report</title><style>
...
function terminalReport(result, color = true) {
  const lines = [
...
function formatResult(result, format, options = {}) {
  switch (format) {
    case "json":
    ...
    case "terminal":
      return terminalReport(result, options.color ?? true);
  }
}
...
async function run() {
  try {
    ...
    const formatValue = input("FORMAT") || "terminal";
    const failOnValue = input("FAIL-ON") || "high";
...
`);
    ...
    if (summaryFile) await (0, import_promises3.appendFile)(summaryFile, `${formatResult(result, "markdown")}
`, "utf8");
    const outputFile = process.env.GITHUB_OUTPUT;
    if (outputFile) await (0, import_promises3.appendFile)(outputFile, `findings=${result.summary.total}
`, "utf8");
